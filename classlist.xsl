...
<xsl:result-document href="{$ns}.txt" method="text">
&lt;ns="<xsl:value-of select="$ns"/>">
<xsl:apply-templates select="t:class" mode="cl">
<xsl:with-param name="ns" select="$ns"/>
</xsl:apply-templates>&lt;/ns&gt;
</xsl:result-document>
</xsl:template>

<xsl:template match="t:class" mode="cl">
<xsl:param name="ns"/>  &lt;class render="0" name="<xsl:value-of select="@name"/>" parent="<xsl:value-of select="@parent"/>"/&gt;
</xsl:template>
